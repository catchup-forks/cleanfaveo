...
class ApiController extends Controller
{
    ...
    public function ticketReply()
    {
        ...
        try {
            ...
            $result = $result->join('users', 'tickets__threads.user_id', '=', 'users.id')
                    ->select('tickets__threads.*', 'users.first_name as first_name')
                    ->orderBy('tickets__threads.id', 'desc')
                    ->first();
    ...
    public function editTicket()
    {
        try {
            $v = \Validator::make($this->request->all(), [
                        ...
                        'tickets__sources'   => 'required|exists:ticket_source,id',
                        'tickets__priorities' => 'required|exists:ticket_priority,priority_id',
            ]);
            if ($v->fails()) {
                $error = $v->errors();
...
//            return response()->json(compact('result'));
            ...
            $result = $this->user->join('tickets', function ($join) {
                $join->on('users.id', '=', 'tickets.user_id')
                    ...
                    ->join('tickets__priorities', 'tickets__priorities.priority_id', '=', 'tickets.priority_id')
                    ->join('tickets__slaplans', 'tickets__slaplans.id', '=', 'tickets.sla')
                    ->join('tickets__helptopics', 'help_topic.id', '=', 'tickets.help_topic_id')
                    ->join('tickets__statuses', 'tickets__statuses.id', '=', 'tickets.status')
                    ->join('tickets__threads', function ($join) {
                        $join->on('tickets.id', '=', 'tickets__threads.ticket_id')
                        ->whereNotNull('title');
                    })
                    ->select('first_name', 'last_name', 'email', 'profile_pic', 'ticket_number', 'tickets.id', 'title', 'tickets.created_at', 'core__departments.name as department_name', 'tickets__priorities.priority as priotity_name', 'tickets__slaplans.name as sla_plan_name', 'tickets_helptopics.topic as help_topic_name', 'tickets__statuses.name as ticket_status_name')
                    ->orderBy('tickets__threads.updated_at', 'desc')
                    ->groupby('tickets.id')
                    ->distinct()
                    ->paginate(10)
                    ->toArray();
...
//            return response()->json(compact('result'));
            ...
            $unassigned = $unassigned->select('tickets__priorities.priority_color as priority_color', \DB::raw('substring_index(group_concat(tickets_threads.title order by tickets_threads.id asc) , ",", 1) as title'), 'tickets.duedate as overdue_date', \DB::raw('count(tickets_attachments.id) as attachment'), \DB::raw('max(tickets_threads.updated_at) as updated_at'), 'user_name', 'first_name', 'last_name', 'email', 'profile_pic', 'ticket_number', 'tickets.id', 'tickets.created_at', 'core__departments.name as department_name', 'tickets_priorities.priority as priotity_name', 'tickets__slaplans.name as sla_plan_name', 'tickets__helptopics.topic as help_topic_name', 'tickets__statuses.name as ticket_status_name')
...
//            return response()->json(compact('result'));
            ...
            $result = $result->select('tickets.duedate as overdue_date', 'tickets__priorities.priority_color as priority_color', \DB::raw('substring_index(group_concat(tickets__threads.title order by tickets__threads.id asc) , ",", 1) as title'), \DB::raw('count(ticket_attachment.id) as attachment'), \DB::raw('max(tickets__threads.updated_at) as updated_at'), 'user_name', 'first_name', 'last_name', 'email', 'profile_pic', 'ticket_number', 'tickets.id', 'tickets.created_at', 'core__departments.name as department_name', 'tickets__priorities.priority as priotity_name', 'tickets__slaplans.name as sla_plan_name', 'tickets__helptopics as help_topic_name', 'tickets__statuses.name as ticket_status_name')
                    ->orderBy('updated_at', 'desc')
                    ->groupby('tickets.id')
                    ->distinct()
                    ->paginate(10)
    ...
    public function getCustomersWith()
    {
        try {
            $users = $this->user
                    ...
                    ->leftJoin('crm__customers', 'crm__customers.id', '=', 'user_assign_organization.org_id')
                    ->where('users.role', 'user')
                    ->select('users.id', 'users.user_name', 'users.first_name', 'users.last_name', 'users.email', 'users.phone_number', 'users.profile_pic', 'crm__customers.name AS company', 'users.active', 'users.ext as telephone_extension', 'users.mobile', 'users.phone_number as telephone', 'users.country_code as mobile_code')
                    ->paginate(10)
                    ->toJson();
    ...
    public function ticketThreads()
    {
        try {
            ...
            $result = $this->user
                    ->rightjoin('tickets__threads', 'tickets__threads.user_id', '=', 'users.id')
                    ->select('tickets__threads.id', 'ticket_id', 'user_id', 'poster', 'source', 'title', 'body', 'is_internal', 'format', 'ip_address', 'tickets__threads.created_at', 'tickets__threads.updated_at', 'users.first_name', 'users.last_name', 'users.user_name', 'users.email', 'users.profile_pic')
                    ->where('ticket_id', $id)
                    ->orderBy('tickets__threads.id', 'desc')
                    ->get()
                    ->toJson();
        ...
        try {
            ...
            $inbox = $this->user->join('tickets', function ($join) {
                $join->on('users.id', '=', 'tickets.user_id')
                    ...
                    ->leftJoin('tickets__attachments', 'ticket_attachment.thread_id', '=', 'tickets__threads.id')
                    ->where('tickets__statuses.name', 'Open');
            if ($user->role == 'agent') {
                $id = $user->id;
                $dept[] = $user->primary_dpt;
            ...
            $inbox = $inbox->select(\DB::raw('max(tickets__threads.updated_at) as updated_at'), 'user_name', 'first_name', 'last_name', 'email', 'profile_pic', 'ticket_number', 'tickets.id', \DB::raw('substring_index(group_concat(tickets__threads.title order by tickets__threads.id asc) , ",", 1) as title'), 'tickets.created_at', 'core__departments.name as department_name', 'tickets__priorities.priority as priotity_name', 'tickets__priorities.priority_color as priority_color', 'tickets__slaplans.name as sla_plan_name', 'tickets__helptopics as help_topic_name', 'tickets__statuses.name as ticket_status_name', 'department.id as department_id', 'users.primary_dpt as user_dpt', \DB::raw('count(ticket_attachment.id) as attachment'), 'tickets.duedate as overdue_date')
                    ->orderBy('updated_at', 'desc')
                    ->groupby('tickets.id')
                    ->distinct()
                    ->paginate(10)
    ...
    public function getTrash()
    {
        try {
            ...
            $trash = $trash->select('tickets__priorities.priority_color as priority_color', \DB::raw('substring_index(group_concat(tickets__threads.title order by tickets__threads.id asc) , ",", 1) as title'), 'tickets.duedate as overdue_date', \DB::raw('count(ticket_attachment.id) as attachment'), \DB::raw('max(tickets__threads.updated_at) as updated_at'), 'user_name', 'first_name', 'last_name', 'email', 'profile_pic', 'ticket_number', 'tickets.id', 'tickets.created_at', 'core__departments.name as department_name', 'tickets__priorities.priority as priotity_name', 'tickets__slaplans.name as sla_plan_name', 'tickets__helptopics as help_topic_name', 'tickets__statuses.name as ticket_status_name')
                    ->orderBy('updated_at', 'desc')
                    ->groupby('tickets.id')
                    ->distinct()
                    ->paginate(10)
        ...
        try {
            ...
            $result = $this->user->join('tickets', function ($join) use ($id) {
                ...
                //->where('user_id', '=', $id);
                    ...
                    ->where('users.id', $id)
                    ->select(
                            'tickets__priorities.priority_color as priority_color', \DB::raw('substring_index(group_concat(tickets__threads.title order by tickets__threads.id asc) , ",", 1) as title'), 'tickets.duedate as overdue_date', \DB::raw('count(ticket_attachment.id) as attachment'), \DB::raw('max(tickets__threads.updated_at) as updated_at'), 'client.user_name', 'client.first_name', 'client.last_name', 'client.email', 'client.profile_pic', 'ticket_number', 'tickets.id', 'tickets.created_at', 'core__departments.name as department_name', 'tickets_priorities.priority as priotity_name', 'tickets__slaplans.name as sla_plan_name', 'tickets__helptopics as help_topic_name', 'tickets__statuses.name as ticket_status_name'
                    )
                    ->orderBy('updated_at', 'desc')
                    ->groupby('tickets.id')
                    ->distinct()
    ...
    public function getMyTicketsUser()
    {
        try {
            ...
            $user = User::where('users.id', $id)
                                            ...
                                            ->select(
                                                    'users.first_name', 'users.last_name', 'users.user_name', 'users.email', 'users.id', 'users.profile_pic', 'users.ban', 'users.active', 'users.is_delete', 'users.phone_number', 'users.ext', 'users.country_code', 'users.mobile', 'crm__customers.name as company'
                                            )->first()->toArray();
            $result = $this->user->join('tickets', function ($join) use ($id) {
                $join->on('users.id', '=', 'tickets.user_id')
                        ->where('user_id', '=', $id);
                    ...
                    ->select('ticket_number', 'tickets.id', 'title', 'tickets__statuses.name as ticket_status_name')
                    ->orderBy('tickets__threads.updated_at', 'desc')
                    ->groupby('tickets.id')
                    ->distinct()
                    ->get()
    ...
    public function getTicketById()
    {
        try {
            ...
            $response = $this->differenciateHelpTopic($query)
                    ...
                    ->leftJoin('tickets__priorities', 'tickets.priority_id', '=', 'tickets__priorities.priority_id')
                    ...
                    ->leftJoin('tickets__sources', 'tickets.source', '=', 'ticket_source.id')
            ...
            $result = $response->addSelect(
                            'users.email', 'users.user_name', 'users.first_name', 'users.last_name', 'tickets.id', 'ticket_number', 'user_id', 'tickets__priorities.priority_id', 'tickets_priorities.priority as priority_name', 'core__departments.name as dept_name', 'tickets__statuses.name as status_name', 'tickets__slaplans.name as sla_name', 'ticket_source.name as source_name', 'tickets__slaplans.id as sla', 'tickets__statuses.id as status', 'lock_by', 'lock_at', 'ticket_source.id as source', 'isoverdue', 'reopened', 'isanswered', 'is_deleted', 'closed', 'reopened_at', 'duedate', 'closed_at', 'tickets.created_at', 'tickets.updated_at', 'tickets__priorities.priority_color as priority_color', 'help_topic.id as helptopic_id', 'tickets__helptopics as helptopic_name', 'tickets__slaplans.grace_period as grace_period')->first();
//            $resultticket_source
            $result2 = $result;
    ...
    public function dependency()
    {
        try {
            ...
            $priority = \DB::table('tickets__priorities')->select('priority', 'priority_id')->get();
            $helptopic = $this->helptopic->select('topic', 'id')->get()->toArray();
            $status = \DB::table('tickets__statuses')->select('name', 'id')->get();
            $source = \DB::table('tickets__sources')->select('name', 'id')->get();
            $statuses = collect($tickets
                            ->join('tickets__statuses', 'tickets.status', '=', 'tickets__statuses.id')
                            ->select('tickets__statuses.name as status', \DB::raw('COUNT(tickets.id) as count'))
                            ->groupBy('tickets__statuses.id')
