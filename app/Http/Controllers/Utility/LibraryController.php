...
class LibraryController extends Controller
{
    ...
    public static function getDatabaseVersion()
    {
        try {
            ...
            if ($database == true) {
                if (Schema::hasColumn('core__settings_system', 'version')) {
                    return \DB::table('core__settings_system')->where('id', '=', '1')->first()->version;
                } else {
                    return 0;
                }
            } else {
    ...
    {
        try {
            if (Schema::hasTable('core__settings_system')) {
                return true;
            }
        } catch (Exception $e) {
            return false;
